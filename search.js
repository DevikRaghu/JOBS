...
jobs.forEach(job => {
 ...
 jobs.forEach(job => {
   const searchTerm = searchLocation;
   const regex = new RegExp(searchTerm, "gi");
   const testString = job.jobLocation;
   const test = regex.test(testString);
   ...
   if (test) {
     ...
     const searchState = async () => {
       const response = await fetch('https://dev-t.vercel.app/idk.json');
       ...
       allCountries.forEach(country => {
         ...
         allStates.forEach(state => {
           ...
           const test = regex.test(testString.name);
           
           if (test) {
             if (!finalCountries.includes(country)) {
       ...
       finalCountries.forEach(finalCountry => {
         const searchTerm = finalCountry.name;
         const regex = new RegExp(searchTerm, "gi");
         const testString = job.jobLocation;
         const test = regex.test(testString);           
